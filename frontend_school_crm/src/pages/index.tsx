    ...
    const calculateStats = async () => {
        try {
            ...
            const paidPayments = payments.filter(
                (p: Payment) => p.status === "paid" || p.status === "partial",
            );

            const totalIncome = paidPayments.reduce((sum, p) => sum + p.amount, 0);

            // Calculate income by payment method
            const cashIncome = paidPayments
                .filter((p: Payment) => p.paymentMethod === "cash")
                .reduce((sum, p) => sum + p.amount, 0);
            const cardIncome = paidPayments
                .filter((p: Payment) => p.paymentMethod === "card")
                .reduce((sum, p) => sum + p.amount, 0);
            const bankIncome = paidPayments
                .filter((p: Payment) => p.paymentMethod === "bank")
                .reduce((sum, p) => sum + p.amount, 0);
            const otherIncome = paidPayments
                .filter(
                    (p: Payment) =>
            ...
            // Calculate expenses ratio for profit calculation
            ...
            const debtors = activeStudents.filter((s) => {
                ...
                const hasNoPaidPayment =
                    ...
                    !studentPayments.some((p) => p.status === "paid");
                return hasNoPaidPayment;
            });

            const unpaidSalaries = teachers.filter((teacher) => {
                const teacherSalaries = salaries.filter(
                    (s: Salary) =>
                ...
                const hasNoPaidSalary =
                    ...
                    !teacherSalaries.some((s) => s.status === "paid");
            ...
            setStats({
                ...
                bankIncome,
                otherIncome,
                cashProfit: cashIncome - totalExpenses * (cashIncome / totalIncome),
                cardProfit: cardIncome - totalExpenses * (cardIncome / totalIncome),
                bankProfit: bankIncome - totalExpenses * (bankIncome / totalIncome),
                otherProfit: otherIncome - totalExpenses * (otherIncome / totalIncome),
            });
        } catch (error) {
        ...
        <div className="space-y-6 md:space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                ...
                >
                    ...
                    <CardContent>
                        <div
                            className={`text-2xl md:text-3xl font-bold ${stats.profit >= 0
            ...
            >
                ...
                <Card className="hover:shadow-lg transition-shadow">
                    ...
                    <CardContent>
                        <div className="space-y-3">
                            <div
                                className={`flex items-center justify-between p-3 md:p-4 rounded-lg ${stats.profit >= 0
            ...
            <div
                className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 animate-fade-in"
                ...
                <Card className="hover:shadow-lg transition-shadow">
                    ...
                    <CardContent>
                        <div className="space-y-2">
                            ...
                            <div
                                className={`text-sm font-semibold ${stats.cashProfit >= 0
                                        ? "text-green-600 dark:text-green-400"
                                        : "text-red-600 dark:text-red-400"
                                    }`}
                            >
                                {t("profit")}: {formatCurrency(stats.cashProfit)}
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="hover:shadow-lg transition-shadow">
                    ...
                    <CardContent>
                        <div className="space-y-2">
                            ...
                            <div
                                className={`text-sm font-semibold ${stats.cardProfit >= 0
                                        ? "text-green-600 dark:text-green-400"
                                        : "text-red-600 dark:text-red-400"
                                    }`}
                            >
                                {t("profit")}: {formatCurrency(stats.cardProfit)}
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="hover:shadow-lg transition-shadow">
                    ...
                    <CardContent>
                        <div className="space-y-2">
                            ...
                            <div
                                className={`text-sm font-semibold ${stats.bankProfit >= 0
                                        ? "text-green-600 dark:text-green-400"
                                        : "text-red-600 dark:text-red-400"
                                    }`}
                            >
                                {t("profit")}: {formatCurrency(stats.bankProfit)}
                            </div>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardContent>
                        <div className="space-y-2">
                            <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">
                                {formatCurrency(stats.otherIncome)}
                            </div>
                            <div
                                className={`text-sm font-semibold ${stats.otherProfit >= 0
                                        ? "text-green-600 dark:text-green-400"
                                        : "text-red-600 dark:text-red-400"
                                    }`}
                            >
                                {t("profit")}: {formatCurrency(stats.otherProfit)}
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
                        ...
                        <div className="p-3 md:p-4 bg-white/60 dark:bg-slate-900/60 rounded-lg backdrop-blur">
                            ...
                            <p
                                className={`text-xl md:text-2xl font-bold ${stats.profit >= 0
