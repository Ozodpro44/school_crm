...
// Redis round-trip. This package is duplicated per-service (see the
// identical file in backend_school_crm/internal/platformsettings) rather
// than shared, matching the existing per-service duplication of
...
// Environment/MaintenanceMode have no server-side consumer here either
// (MaintenanceMode is a purely client-side localStorage flag, Environment
// is display-only) — they round-trip only so the JSON shape matches what
// backend_school_crm writes.
