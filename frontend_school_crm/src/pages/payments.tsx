...
import { paymentsDB, studentsDB, classesDB, branchesDB, usersDB, monthArchivesDB } from "@/lib/storage";
...
export default function PaymentsPage() {
  ...
  const canDeletePayments = canEditPayments && getCurrentUser()?.role !== "manager";
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Prevent creating a new payment when the student is already fully paid for this month/year
    ...
    if (!editingPaymentId && periodPaidTotal >= monthlyPaymentValue) {
      toast({ title: t("alreadyFullyPaidForPeriod") });
      return;
    }

    if (editingPaymentId) {
      // update existing payment
      // if editing and moving to a different period, ensure target period isn't already fully paid
      ...
      if (existingPayment) {
        ...
        // if changing period, exclude the current payment from the sum check
        const paidTotalExcludingCurrent = paymentsDB
          .getAll()
          .filter(
            (p) =>
              p.studentId === formData.studentId &&
        ...
        if (paidTotalExcludingCurrent >= monthlyPaymentValue) {
          toast({ title: t("alreadyFullyPaidForPeriod") });
          return;
        }
      }
    ...
    // After creating/updating, recompute aggregated status for the student/month/year
  ...
  const handleMarkPaid = (
    ...
    // recompute status for this payment's student/month/year
    const payment = payments.find((p) => p.id === id);
    if (payment) {
      recomputePaymentStatus(payment.studentId, payment.month, payment.year);
    }
    loadData();
  };
  ...
  const handleDelete = (id: string) => {
    ...
    // after delete, recompute statuses for the affected student/month/year
    // find deleted payment in previous state (payments state still has it)
    const deleted = payments.find((p) => p.id === id);
    if (deleted) {
      // perform delete (already done) then recompute
      recomputePaymentStatus(deleted.studentId, deleted.month, deleted.year);
    }
    loadData();
  ...
  const handleBulkPayment = () => {
    ...
    selectedStudentIds.forEach((studentId) => {
      ...
      if (paidTotal >= student.monthlyPayment) {
        skipped.push(studentId);
        return; // skip already fully-paid student
      }
    ...
    if (skipped.length > 0) {
      toast({ title: t("bulkSkippedFullyPaid"), variant: "success" });
    }
  ...
  const filteredPayments = payments.filter((payment) => {
    ...
    const visibilityMatch = isMonthVisible(payment.month, payment.year);
                            ...
                            >
                              ...
                              <div className="flex-1">
                                ...
                                <p className="text-sm text-slate-500 dark:text-slate-400">
                                  {getClassName(student.id)} • {formatCurrency(student.monthlyPayment)}
                                  /{t("month")}
                                </p>
                              </div>
                            </div>
                ...
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-800">
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-slate-900 dark:text-slate-100">
                        <span className="text-sm">{payment.createdBy ? getUserName(payment.createdBy) : "-"}</span>
                      </td>
                      <td className="py-3 px-4">
                        <div className="flex items-center justify-end gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                               ...
                               title={canEditPayments ? t("edit") : t("noPermission")}
                             >
                             ...
                             <Button
                               ...
                               disabled={!canDeletePayments}
                               title={canDeletePayments ? t("delete") : t("noPermission")}
                             >
              ...
              {filteredPayments.length > 0 && (
               <div className="flex items-center justify-between mt-6 pt-6 border-t border-slate-200 dark:border-slate-800">
                 <div className="text-sm text-slate-600 dark:text-slate-400">
                   {t("showing")} {startIndex + 1} - {Math.min(startIndex + itemsPerPage, filteredPayments.length)} {t("of")} {filteredPayments.length}
                 </div>
                 <div className="flex gap-2">
                   <Button
                     variant="outline"
                     size="sm"
                     onClick={() => router.push(`/payments?page=${Math.max(1, currentPage - 1)}`)}
                   ...
                   <Button
                     variant="outline"
                     size="sm"
                     onClick={() => router.push(`/payments?page=${Math.min(totalPages, currentPage + 1)}`)}
                  ...
                  <div className="text-center border-b border-dashed border-slate-300 dark:border-slate-700 print:border-slate-300 pb-2 mb-2">
                    ...
                    <p className="text-xs text-slate-600 dark:text-slate-400 print:text-black print:text-opacity-70">
                      {(() => {
                        const user = getCurrentUser();
                        if (user?.branchId) {
                          const branch = branchesDB.getAll().find(b => b.id === user.branchId);
