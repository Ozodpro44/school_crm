// Package middleware provides JWT authentication for payment_service.
//
// api_gateway already validates the JWT and forwards trusted X-User-*
// headers, so in normal operation this is defense-in-depth. But
// payment_service's HTTP port is not currently firewalled to gateway-only
// traffic, and this service handles financial data (create/update/delete
// payments, bulk-pay, subscriptions) — without its own check, anyone who can
// reach the service directly bypasses authentication entirely.
package middleware
